...
class TestTicketEnricher:
    ...
    async def test_create_ticket(
        ...
        mock_jira.create_ticket.return_value = IdeaResult(
            jira_key="PROJ-50",
        ...
        result = await enricher.create_ticket("add dark mode to the settings page", auto_enrich=False)
        ...
        idea = mock_jira.create_ticket.call_args[0][0]
        assert idea.priority == "Medium"
        assert idea.issue_type == "Story"
        assert "frontend" in idea.labels

    @patch("giga_mcp_server.enrichment.anthropic.AsyncAnthropic")
    async def test_create_ticket_with_auto_enrich(
        self,
        MockAnthropic: MagicMock,
        mock_jira: AsyncMock,
        mock_settings: MagicMock,
        ...
        # First call: create_ticket prompt, second+: analyze_ticket prompt
        story_response = {
        ...
        enricher = TicketEnricher(mock_jira, mock_settings)
        result = await enricher.create_ticket("login is crashing when i hit submit", auto_enrich=True)
